...
// banner) at the encounter position, gizmo-movable in Edit Mode. The actual battle still fires via a
// battleTrigger / bossGate; this just lets you see + position where an encounter lives.
export const EncounterMarkerRenderer = ({ areaId }: { areaId: string }) => {
  ...
  return <>{here.map((enc) => <EncounterVisual key={enc.id} areaId={areaId} enc={enc} />)}</>;
};
...
const EncounterVisual = ({ areaId, enc }: { areaId: string; enc: EditorEncounter }) => {
  const key = objKey(areaId, 'encounter', enc.id);
  const lead = getCombatant(enc.enemyTeam[0]?.combatantId);
  const model = lead?.modelAssetId;
  const color = lead?.color ?? '#ef4444';
  return (
    <EditableObject objKey={key} base={{ position: enc.position ?? [0, 0, 4] }}>
