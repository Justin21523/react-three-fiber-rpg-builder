...
export const QuestEditorTab = () => {
  ...
  return (
    <div className="space-y-3 text-xs">
      ...
      {quests.length === 0 ? (
        <p className="rounded bg-slate-900/60 px-2 py-2 text-[11px] text-slate-400">No quests yet. “+ New Quest” registers it into the runtime immediately — it shows in the Quest tracker once a giver/dialogue starts it (or use a dialogue choice with the startQuest effect).</p>
      ) : (
      ...
      {sel && (
        <div className="space-y-2 rounded border border-slate-700 bg-slate-900/50 p-2">
          ...
          <div className="space-y-1 rounded border border-slate-700/70 bg-slate-950/40 p-2">
            <div className="flex items-center justify-between"><span className="text-[10px] font-bold uppercase text-cyan-300">Objectives</span>
          ...
          {/* Reward */}
          <div className="space-y-1 rounded border border-slate-700/70 bg-slate-950/40 p-2">
            <span className="text-[10px] font-bold uppercase text-amber-300">Reward</span>
            ...
            <div className="flex items-center justify-between"><span className="text-[10px] text-slate-500">Item rewards</span>
      ...
      <div className="space-y-1 rounded border border-slate-700 bg-slate-900/50 p-2">
        <div className="flex items-center justify-between"><h4 className="text-[10px] font-bold uppercase text-emerald-300">Items</h4>
          <button className={btn} onClick={() => useEditorQuestStore.getState().newItem()}>+ New Item</button>
        </div>
        {items.length === 0 && <p className="text-[10px] text-slate-500">No authored items. Seed items (Old Key, Healing Herb) are always available.</p>}
        {items.map((it) => (
